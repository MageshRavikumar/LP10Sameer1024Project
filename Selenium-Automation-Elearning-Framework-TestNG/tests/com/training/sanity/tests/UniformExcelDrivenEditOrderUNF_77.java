...
public class UniformExcelDrivenEditOrderUNF_77 {
	...
	public void modifyOrder(String orderid, String product, String qty) {
	
		orderid = String.valueOf((int) Double.parseDouble(orderid));
		product = product.trim();
		qty = String.valueOf((int) Double.parseDouble(qty));
		System.out.println("DataUsed: "+orderid+" "+product+" "+qty);
		
		filterOrderPOM.orderList();
		filterOrderPOM.searchByID(orderid);
		selectEdit();			
		editOrder(orderid, product, qty);
	...
	private void editOrder(String orderid, String newproduct, String qty) {
		String product;
		...
		editOrderPOM.addProduct(newproduct);
		...
		editOrderPOM.selectProduct(newproduct);
		editOrderPOM.addQuantity(qty);
		...
		Assert.assertTrue(order.getText().contains(newproduct));
		...
		//ts.captureScreenShot("UNF_077_ProductSaved");
